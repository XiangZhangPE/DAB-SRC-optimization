...
low_power_range = linspace(0.015, 0.45, 25);
...
Vo_values = 80:5:120; % Adjust the range as needed
% Define different initial conditions for each region
low_power_initial = [0.06, 0.7, 0.95, 1/1.1];
...
for v_idx = 1:length(Vo_values)
    ...
    for i = 1:length(PoN_values)
        ...
        % 保证单调递增
        if i > 1
            diff_vars = optimal_vars - optimal_points(i-1,:,v_idx);
            % 强制调整为单调递增
            optimal_vars(diff_vars < 0) = optimal_points(i-1, diff_vars < 0, v_idx);
            % 限制增量不超过阈值
            excessive_increment = diff_vars > distance_threshold;
            optimal_vars(excessive_increment) = optimal_points(i-1, excessive_increment, v_idx) + distance_threshold;
        end

        % Store results in 3D arrays
        optimal_points(i,:,v_idx) = optimal_vars;
        optimal_power(i,v_idx) = f_function(DoF, optimal_vars, Vin, Vo);
        optimal_Irms(i,v_idx) = g_function(DoF, optimal_vars, Vin, Vo);
        ...
        actual_PoN_values(i,v_idx) = f_function(DoF, optimal_vars, Vin, Vo); % 使用 f_function 计算实际 PoN 值
    ...
    fprintf('⚡️ Vo = %.2f finished \n', Vo);
end

% 大循环结束后，统一剔除未初始化的点
valid_indices = actual_PoN_values ~= 0; % 找到所有有效点的索引
...
figure(1);
hold on;
mesh(PoN_grid, Vo_grid, Ioffp12', 'FaceColor', 'r', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5, 'DisplayName', 'I_{off,P12}');
mesh(PoN_grid, Vo_grid, Ioffp34', 'FaceColor', 'b', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5, 'DisplayName', 'I_{off,P34}');
mesh(PoN_grid, Vo_grid, Ioffs12', 'FaceColor', 'm', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5, 'DisplayName', 'I_{off,S12}');
mesh(PoN_grid, Vo_grid, Ioffs34', 'FaceColor', 'c', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5, 'DisplayName', 'I_{off,S34}');
mesh(PoN_grid, Vo_grid, ILmax', 'FaceColor', 'y', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'DisplayName', 'I_{L,max}');
...
% Subplot 1: Dp
subplot(2, 2, 1);
mesh(PoN_grid, Vo_grid, squeeze(optimal_points(:, 1, :))', 'FaceColor', 'r', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5);
xlabel('Normalized output power PoN', 'Interpreter', 'latex');
ylabel('Voltage ratio M = Vo/Vin', 'Interpreter', 'latex');
zlabel('Dp', 'Interpreter', 'latex');
title('Phase-shift ratio Dp', 'Interpreter', 'latex');
grid on;
view(3);

% Subplot 2: Dy1
subplot(2, 2, 2);
mesh(PoN_grid, Vo_grid, squeeze(optimal_points(:, 2, :))', 'FaceColor', 'g', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5);
xlabel('Normalized output power PoN', 'Interpreter', 'latex');
ylabel('Voltage ratio M = Vo/Vin', 'Interpreter', 'latex');
zlabel('Dy1', 'Interpreter', 'latex');
title('Phase-shift ratio Dy1', 'Interpreter', 'latex');
grid on;
view(3);

% Subplot 3: Dy2
subplot(2, 2, 3);
mesh(PoN_grid, Vo_grid, squeeze(optimal_points(:, 3, :))', 'FaceColor', 'b', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5);
xlabel('Normalized output power PoN', 'Interpreter', 'latex');
ylabel('Voltage ratio M = Vo/Vin', 'Interpreter', 'latex');
zlabel('Dy2', 'Interpreter', 'latex');
title('Phase-shift ratio Dy2', 'Interpreter', 'latex');
grid on;
view(3);
...
if DoF >= 4
    subplot(2, 2, 4);
    mesh(PoN_grid, Vo_grid, squeeze(optimal_points(:, 4, :))', 'FaceColor', 'y', 'EdgeColor', [0.9 0.9 0.9], 'EdgeAlpha', 0.3,  'FaceAlpha', 0.5);
    xlabel('Normalized output power PoN', 'Interpreter', 'latex');
    ylabel('Voltage ratio M = Vo/Vin', 'Interpreter', 'latex');
    zlabel('r', 'Interpreter', 'latex');
    title('Phase-shift ratio r', 'Interpreter', 'latex');
    grid on;
    view(3);
end
