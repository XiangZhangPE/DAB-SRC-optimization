...
function [optimal_points, optimal_power, optimal_Irms, Ioffp12, Ioffp34, Ioffs12, Ioffs34, ILmax, actual_PoN_values] = ...
    ...
    for v_idx = 1:p
        % 检查第一个点（边缘点）
        if m > 2 && valid_indices(1, v_idx)
            % 获取接下来的两个有效点
            next_idx1 = 2;
            while next_idx1 <= m && ~valid_indices(next_idx1, v_idx)
                next_idx1 = next_idx1 + 1;
            end
            
            if next_idx1 <= m
                next_idx2 = next_idx1 + 1;
                while next_idx2 <= m && ~valid_indices(next_idx2, v_idx)
                    next_idx2 = next_idx2 + 1;
                end
                
                if next_idx2 <= m
                    % 检查第一个点与后两个点的差异
                    diff_next1 = abs(optimal_points(1,:,v_idx) - optimal_points(next_idx1,:,v_idx));
                    diff_next2 = abs(optimal_points(1,:,v_idx) - optimal_points(next_idx2,:,v_idx));
                    ...
                    % 第一个点如果与后面两个点都有明显差异，标记为突变点
                    if any(diff_next1 > edge_thresholds) && any(diff_next2 > edge_thresholds)
                        outlier_mask(1, v_idx) = true;
                    end
                end
            end
        end
        ...
        % 检查最后一个点（边缘点）
        if m > 2 && valid_indices(m, v_idx)
            % 获取前面的两个有效点
            prev_idx1 = m - 1;
            while prev_idx1 >= 1 && ~valid_indices(prev_idx1, v_idx)
                prev_idx1 = prev_idx1 - 1;
            end
            
            if prev_idx1 >= 1
                prev_idx2 = prev_idx1 - 1;
                while prev_idx2 >= 1 && ~valid_indices(prev_idx2, v_idx)
                    prev_idx2 = prev_idx2 - 1;
                end
                
                if prev_idx2 >= 1
                    % 检查最后一个点与前两个点的差异
                    diff_prev1 = abs(optimal_points(m,:,v_idx) - optimal_points(prev_idx1,:,v_idx));
                    diff_prev2 = abs(optimal_points(m,:,v_idx) - optimal_points(prev_idx2,:,v_idx));
                    ...
                    % 最后一个点如果与前面两个点都有明显差异，标记为突变点
                    if any(diff_prev1 > edge_thresholds) && any(diff_prev2 > edge_thresholds)
                        outlier_mask(m, v_idx) = true;
                    end
                end
            end
        end
        ...
        % 检查中间点的突变
        for i = 2:m-1
            if valid_indices(i, v_idx)
                % 查找前一个和后一个有效点
                prev_valid = i - 1;
                while prev_valid >= 1 && ~valid_indices(prev_valid, v_idx)
                    prev_valid = prev_valid - 1;
                end
                
                next_valid = i + 1;
                while next_valid <= m && ~valid_indices(next_valid, v_idx)
                    next_valid = next_valid + 1;
                end
                
                % 确保找到了有效的相邻点
                if prev_valid >= 1 && next_valid <= m
                    % 检查当前点与相邻点的差异
                    diff_prev = abs(optimal_points(i,:,v_idx) - optimal_points(prev_valid,:,v_idx));
                    diff_next = abs(optimal_points(i,:,v_idx) - optimal_points(next_valid,:,v_idx));
        ...
        for i = 1:m
            if outlier_mask(i, v_idx)
                ...
                % 边缘点特殊处理
                if i == 1 && next_valid <= m
                    % 第一个点: 直接使用下一个有效非突变点替代
                    optimal_points(i,:,v_idx) = optimal_points(next_valid,:,v_idx);
                elseif i == m && prev_valid >= 1
                    % 最后一个点: 直接使用前一个有效非突变点替代
                    optimal_points(i,:,v_idx) = optimal_points(prev_valid,:,v_idx);
                elseif prev_valid >= 1 && next_valid <= m
                    % 中间点: 使用线性插值替换
                    weight = (i - prev_valid) / (next_valid - prev_valid);
                    optimal_points(i,:,v_idx) = (1-weight) * optimal_points(prev_valid,:,v_idx) + weight * optimal_points(next_valid,:,v_idx);
                elseif prev_valid >= 1
                    % 只有前面有有效点
                    optimal_points(i,:,v_idx) = optimal_points(prev_valid,:,v_idx);
                elseif next_valid <= m
                ...
                % 更新所有相关的计算值
                if (prev_valid >= 1 || next_valid <= m) && valid_indices(i, v_idx)
                    Vo = Vo_values(v_idx);
                    interp_vars = optimal_points(i,:,v_idx);
